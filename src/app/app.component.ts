...
import * as productActions from './pages/products-page/products.actions';
import * as bookingActions from './pages/product-detail-page/product-detail-page.actions';
import {
  AngularFirestore,
...
export class AppComponent implements OnInit {
  ...
  ngOnInit() {
    this.productsCollection
      ...
      .subscribe((products) => {
        this.store.dispatch(
          productActions.getProductsSucceeded({ payload: products })
        );
      });
    ...
    this.bookingsCollection
      ...
      .subscribe((bookings) => {
        this.store.dispatch(
          bookingActions.getBookingsSucceeded({ payload: bookings })
        );
      });
    ...
    this.afAuth.authState.subscribe((auth) => {
      if (auth) {
        this.store.dispatch(
          authActions.authenticated({ payload: this.fromFirebaseUser(auth) })
        );
      } else {
